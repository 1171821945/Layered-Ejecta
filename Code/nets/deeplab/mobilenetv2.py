    ...
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        if expand_ratio == 1:
            self.conv = nn.Sequential(
                ...
                nn.ReLU6(inplace=True),
                #-----------------------------------#
                #   Adjust the number of channels using 1x1 convolution
                #-----------------------------------#
            ...
            self.conv = nn.Sequential(
                #-----------------------------------#
                #   Using 1x1 convolution to increase the number of channels
                #-----------------------------------#
                ...
                nn.ReLU6(inplace=True),
                #-----------------------------------#
                #   Using 1x1 convolution to reduce the number of channels
                #-----------------------------------#
                nn.Conv2d(hidden_dim, oup, 1, 1, 0, bias=False),
                BatchNorm2d(oup),
            )
...
class MobileNetV2(nn.Module):
    def __init__(self, n_class=1000, input_size=224, width_mult=1.):
        ...
        interverted_residual_setting = [
            # t, c, n, s
            # t: Expansion ratio c: Number of output channels n: Number of block repetitions s: Stripes
        ...
        for t, c, n, s in interverted_residual_setting:
            ...
            for i in range(n):
                # For each block group, the step size of the first one is s, and the rest are 1, which ensures that each block group will only have one downsampling
                if i == 0:
