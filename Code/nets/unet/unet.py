...
class AttentionBlock(nn.Module):
    def __init__(self, skip_x_channels, x_channels, int_channels):
...
class unetUp(nn.Module):
    # in_size:channels (x) + channels (skip_x)
    # out_size next layer's x
    # channels (x) = in_size - out_size
    # channels (skip_x) = out_size
    def __init__(self, in_size, out_size, skip_x_size, x_size, is_attention = True):
        super(unetUp, self).__init__()
        self.is_attention = is_attention
        if is_attention:
            self.attention = AttentionBlock(skip_x_size, x_size, int((in_size - out_size) / 2))
...
class Unet(nn.Module):
    ...
    def __init__(self, num_classes = 21, pretrained = False, backbone = 'vgg', is_attention = False, is_batch = False, is_body_batch=False, vgg_cfg_ind = 'D'):
        ...
        if backbone == 'vgg':
            ...
            x_channels = [128, 256, 512, 2048]
            skip_x_channels = [64, 256, 512, 1024]
        ...
        # upsampling
        # 64,64,512
        self.up_concat4 = unetUp(in_filters[3], out_filters[3], skip_x_channels[3], x_channels[3], is_attention)
        # 128,128,256
        self.up_concat3 = unetUp(in_filters[2], out_filters[2], skip_x_channels[2], x_channels[2], is_attention)
        # 256,256,128
        self.up_concat2 = unetUp(in_filters[1], out_filters[1], skip_x_channels[1], x_channels[1], is_attention)
        # 512,512,64
        self.up_concat1 = unetUp(in_filters[0], out_filters[0], skip_x_channels[0], x_channels[0], is_attention)
