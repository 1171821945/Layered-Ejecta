...
class CAFM(nn.Module):  # Cross Attention Fusion Module
    def __init__(self, channels):
        ...
        self.avg1 = nn.Conv2d(channels, channels, 1, stride=1, padding=0)
        self.avg2 = nn.Conv2d(channels, channels, 1, stride=1, padding=0)
        self.max1 = nn.Conv2d(channels, channels, 1, stride=1, padding=0)
        self.max2 = nn.Conv2d(channels, channels, 1, stride=1, padding=0)

        self.avg11 = nn.Conv2d(channels, channels, 1, stride=1, padding=0)
        self.avg22 = nn.Conv2d(channels, channels, 1, stride=1, padding=0)
        self.max11 = nn.Conv2d(channels, channels, 1, stride=1, padding=0)
        self.max22 = nn.Conv2d(channels, channels, 1, stride=1, padding=0)
...
class LinAngularXCA_CA(nn.Module):
    def __init__(self):
        ...
        self.la = LinAngularAttention(in_channels=512)
        self.xa = XCA(dim=512)
        self.cafm = CAFM()
    ...
    def forward(self, x):
        ...
        result = to_4d(result, 32, 32)
        #
        return result

if __name__ == '__main__':
    block = CAFM(512)
    input1 = torch.rand(32, 512, 32, 32)
    input2 = torch.rand(32, 512, 32, 32)
    output1, output2 = block(input1, input2)  # 获取两个输入的输出
