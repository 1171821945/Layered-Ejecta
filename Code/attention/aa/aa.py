...
def to_3d(x, h, w):
    return rearrange(x, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', w=w, h=h)

def to_4d(x, h, w, p1, p2):
...
class AgentAttention(nn.Module):
    def __init__(self, dim, num_patches, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 sr_ratio=1, agent_num=64, **kwargs):
        super().__init__()
    ...
    def forward(self, x):
       # _0, _1, H, W = x.shape
        ...
        if self.sr_ratio > 1:
            x_ = x.permute(0, 2, 1).reshape(b, c, h, h)
        ...
        agent_tokens = self.pool(q.reshape(b, h, h, c).permute(0, 3, 1, 2)).reshape(b, c, -1).permute(0, 2, 1)
        ...
        v = v.transpose(1, 2).reshape(b, h // self.sr_ratio, h // self.sr_ratio, c).permute(0, 3, 1, 2)
        if self.sr_ratio > 1:
            v = nn.functional.interpolate(v, size=(h, h), mode='bilinear')
...
if __name__ == '__main__':
    ...
    H, W = 64, 64
    num_patches = 32*32
    block = AgentAttention(dim=dim*4, num_patches=num_patches)
